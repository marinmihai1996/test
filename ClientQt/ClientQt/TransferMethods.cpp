...
bool Client::RequestFile(std::string FileName, std::string groupName) {
	...
	string mes = "FileExists ";
	mes.append(std::to_string(this->ID));
	mes.append(" ");
	mes.append(path);
	SendString(mes, true);
...
bool Client::UpdateFile(std::string FileName, std::string groupName)
{
	...
	/*std::string path = "C:/Users/Maria/Documents/git/test/Server/Server/";
	path.append(groupName);
	path.append("/");
	path.append(FileName);
	fileInServer.outfileStream.open(path, std::ios::binary); //open file to write file to
	fileInServer.fileName = FileName; //save file name
	fileInServer.bytesWritten = 0; //reset byteswritten to 0 since we are working with a new file
	if (!file.outfileStream.is_open()) //if file failed to open...
	{
	std::cout << "ERROR: Function(Client::RequestFile) - Unable to open file: " << FileName << " for writing.\n";
	return false;
